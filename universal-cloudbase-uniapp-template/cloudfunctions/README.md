...
11. [注意事项](#11-注意事项)
12. [定时任务配置](#12-定时任务配置)
13. [HTTP云函数与回调接口](#13-http云函数与回调接口)
14. [参考资料](#14-参考资料)
...
├─────────────────────────────────────────────────────────────┤
│  业务逻辑层                                                    │
│  - 双标识体系：_openid（CloudBase uid）+ openid（微信openid）│
...
│  数据存储层                                                    │
│  - 28张业务表                                                  │
│  - _openid 字段（CloudBase uid，用户数据隔离与接口鉴权）      │
│  - openid 字段（真实微信 openid，用于微信支付等微信 API）     │
│  - 索引优化                                                    │
...
> ⚠️ **重要**：本项目使用 CloudBase JS SDK（`@cloudbase/js-sdk`）调用云函数，**不是**微信原生 `wx.cloud`。因此 `cloud.getWXContext().OPENID` 在云函数中返回 `undefined`，需要采用双标识体系。

### 2.1 为什么需要双标识？

| 问题 | 说明 |
|------|------|
| CloudBase JS SDK 调用云函数时，`wx-server-sdk` 的 `getWXContext()` 无法获取微信 OPENID | 返回 `undefined` |
| CloudBase Node SDK 的 `auth.getUserInfo()` 返回的是 CloudBase uid | 不是微信 openid |
| 微信支付等微信 API 需要真实的微信 openid | CloudBase uid 无法用于微信支付 |

### 2.2 双标识定义

| 字段 | 存储内容 | 获取方式 | 职责 |
|------|---------|---------|------|
| `_openid` | CloudBase uid | `auth.getUserInfo()` | **接口鉴权** + **用户数据隔离** |
| `openid` | 真实微信 openid | 登录时 `code2session` 换取 | **微信支付** + **微信 API 调用** |

### 2.3 各字段职责详解

#### `_openid`（CloudBase uid）— 鉴权与数据隔离

```
职责范围：
├── 接口鉴权（checkClientAuth / checkAdminAuth）
│   └── 通过 _openid 查找用户，验证身份
├── 用户数据隔离
│   └── CloudBase 安全规则基于 _openid 控制数据读写权限
├── 数据库查询
│   └── WHERE _openid = ? 定位用户数据
└── 所有非微信 API 的用户操作
    └── 个人资料更新、订单查询、课程管理等
```

- **获取时机**：每次云函数调用时，由 `index.js` 入口通过 `auth.getUserInfo()` 自动获取
- **存储位置**：`users._openid`，以及所有需要用户数据隔离的表的 `_openid` 字段
- **使用场景**：所有接口的用户识别和权限验证

#### `openid`（真实微信 openid）— 微信 API

```
职责范围：
├── 微信支付
│   └── cloud.cloudPay.unifiedOrder({ openid: 真实openid })
├── 微信消息推送
│   └── 模板消息、订阅消息的 touser 字段
├── 微信用户信息
│   └── 需要微信身份的第三方 API
└── 其他微信开放能力
    └── 任何需要真实微信 openid 的场景
...
- **获取时机**：仅在用户 **登录时** 通过 `uni.login()` 获取 code，后端调用 `jscode2session` 换取
- **存储位置**：`users.openid`
- **使用场景**：微信支付、微信消息推送等需要真实微信身份的场景
...
│  ┌──────┐               ┌──────────┐              ┌──────────┐  │
│  │      │──uni.login()──│          │              │          │  │
│  │      │   获取 code   │          │──code2session→│          │  │
│  │ 小程序│              │ login.js │←─真实openid──│ 微信 API │  │
│  │      │              │          │              │          │  │
│  │      │←─登录成功────│          │              │          │  │
│  └──────┘               └────┬─────┘              └──────────┘  │
│                              │                                   │
...
│                    │     数据库 users    │                       │
│                    │                    │                       │
│                    │  _openid = CB uid  │← auth.getUserInfo()   │
│                    │  openid  = 微信id  │← code2session         │
│                    └────────────────────┘                       │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  普通接口调用流程（updateProfile / getProfile / createOrder 等） │
│                                                                  │
│  前端                     云函数入口                             │
│  ┌──────┐               ┌──────────────┐                       │
│  │      │──callFunction─│ index.js     │                       │
│  │ 小程序│              │              │                       │
│  │      │              │ auth.getUserInfo()                    │
│  │      │              │   → CloudBase uid                     │
│  │      │              │   → checkClientAuth(_openid)          │
│  │      │              │   → 找到用户 → 执行业务逻辑           │
│  │      │←─返回结果────│              │                       │
│  └──────┘               └──────────────┘                       │
│                                                                  │
│  ⚠️ 注意：普通接口不获取微信 openid，仅用 _openid 鉴权         │
└─────────────────────────────────────────────────────────────────┘
...
│  │ createPayment│         │    users     │                     │
│  │              │──查询──→│              │                     │
│  │  需要真实    │←openid──│ openid=微信id│                     │
│  │  微信openid  │         └──────────────┘                     │
│  │              │                                               │
│  │  cloud.cloudPay.unifiedOrder({                              │
│  │    openid: user.openid  ← 从数据库读取真实微信 openid       │
│  │  })                                                         │
│  └──────────────┘                                               │
└─────────────────────────────────────────────────────────────────┘
```
...
#### 入口文件获取 CloudBase uid（user/index.js）
...
exports.main = async (event, context) => {
  let OPENID = event.test_openid; // 测试用

  if (!OPENID) {
    // 获取 CloudBase uid（不是微信 openid）
  ...
  // login 接口：传递 OPENID 但不强制鉴权
  if (action === 'login') {
    return await loginHandler(event, { OPENID });
  }

  // 其他接口：通过 _openid 鉴权
  const user = await checkClientAuth(OPENID); // 用 _openid 查库
  return await handler(event, { OPENID, user });
};
```

#### 登录接口获取真实微信 openid（login.js）
...
module.exports = async (event, context) => {
  const { OPENID: cloudbaseUid } = context; // CloudBase uid
  const { code } = event;

  // 通过 code 换取真实微信 openid
  ...
  const wxOpenid = result.openid; // 真实微信 openid

  // 存入数据库：双标识
  await insert('users', {
    _openid: cloudbaseUid,  // CloudBase uid → 鉴权 + 数据隔离
    openid: wxOpenid,       // 微信 openid → 微信支付 + 微信 API
    // ...其他字段
  });
};
```

#### 微信支付使用真实 openid（createPayment.js）

```javascript
module.exports = async (event, context) => {
  const { user } = context;

  // 从数据库用户记录中读取真实微信 openid
  const payResult = await cloud.cloudPay.unifiedOrder({
    body: '商品描述',
    outTradeNo: orderNo,
    totalFee: totalFee,
    openid: user.openid,  // ✅ 使用数据库中的真实微信 openid
    // ❌ 不要用 context.OPENID（那是 CloudBase uid）
  });
};
```
...
| 注意点 | 说明 |
|--------|------|
| ⚠️ 不要混淆两个标识 | `_openid` ≠ `openid`，它们存储不同的值 |
| ⚠️ 微信支付必须用 `openid` | 使用 `_openid`（CloudBase uid）会报 `invalid wx openapi access_token` |
| ⚠️ 鉴权必须用 `_openid` | CloudBase 安全规则基于 `_openid` |
| ⚠️ openid 仅登录时获取 | 不是每次请求都获取，从数据库读取即可 |
| ⚠️ 首次登录数据迁移 | 老用户首次使用新登录流程时，login.js 会自动将真实 openid 写入 `openid` 字段 |
...
证书序列号：6D4BEDC9337BD2C074748CCF667311E0A5BD9C7E
证书串：
-----BEGIN PRIVATE KEY-----
...
**双标识体系：区分 CloudBase uid 和微信 openid**
...
exports.main = async (event, context) => {
  const { action } = event;
  let OPENID = event.test_openid; // 测试环境使用
  
  // ✅ 使用 auth.getUserInfo() 获取 CloudBase uid（用于鉴权和数据隔离）
  if (!OPENID) {
    const userInfo = auth.getUserInfo();
    if (userInfo && userInfo.uid) {
      OPENID = userInfo.uid;  // 这是 CloudBase uid，对应数据库的 _openid 字段
    }
  }
  
  console.log(`[${action}] CloudBase uid (OPENID):`, OPENID?.slice(-6) || 'undefined');
  
  // ❌ 不要从前端接收 openid（可以被伪造）
  // const { openid } = event; // 错误！
  
  // ⚠️ 注意：这里的 OPENID 是 CloudBase uid，不是微信 openid
  // - CloudBase uid：用于用户识别、权限验证、数据隔离（存在 _openid 字段）
  // - 微信 openid：仅在登录时通过 code2session 获取并存储（存在 openid 字段）
  //               用于微信支付、消息推送等微信 API 调用
}
```

**身份获取场景说明：**

| 场景 | 获取方式 | 获取内容 | 存储字段 | 用途 |
|------|---------|---------|---------|------|
| 云函数入口（所有action） | `auth.getUserInfo()` | CloudBase uid | `_openid` | 用户识别、权限验证 |
| 登录接口内部 | `jscode2session` API | 真实微信 openid | `openid` | 微信支付、消息推送 |
| 支付接口内部 | 从数据库读取 | 真实微信 openid | `openid` | 传递给微信支付 API |
...
// ✅ 正确写法（使用 CloudBase uid）
const cloudbase = require('@cloudbase/node-sdk');
const app = cloudbase.init({ env: cloudbase.SYMBOL_CURRENT_ENV });
const auth = app.auth();

exports.main = async (event, context) => {
  // 获取 CloudBase uid（用于鉴权）
  const userInfo = auth.getUserInfo();
  const OPENID = userInfo?.uid;  // 这是 CloudBase uid，对应 _openid 字段
  
  // ⚠️ 重要提示：
  // 1. OPENID 是 CloudBase uid，用于所有接口的权限验证
  // 2. 真实的微信 openid 仅在登录时通过 code2session 获取
  // 3. 需要微信 openid 时（如支付），从数据库的 openid 字段读取
}
```
...
```javascript
// ✅ 接口鉴权：使用 CloudBase uid (_openid)
const user = await checkClientAuth(OPENID); // OPENID 是 CloudBase uid
const orders = await query(
  'SELECT * FROM orders WHERE _openid = ?',
  [OPENID]  // 用 _openid 进行数据隔离
);

// ✅ 微信支付：使用真实微信 openid (openid)
const { data: user } = await db
  .from('users')
  .select('openid')  // 从数据库读取真实的微信 openid
  .eq('_openid', OPENID)  // 通过 CloudBase uid 查询
  .single();

const paymentResult = await cloud.cloudPay.unifiedOrder({
  openid: user.openid,  // 使用真实的微信 openid
  // ...
});

// ❌ 错误：混用两种标识
const paymentResult = await cloud.cloudPay.unifiedOrder({
  openid: OPENID,  // 错误！OPENID 是 CloudBase uid，不能用于支付
  // ...
});
```
...
| 问题 | 解决方案 |
|-----|---------|
| 如何获取用户 openid？ | **双标识体系：**<br>1. **CloudBase uid (_openid)**：在云函数入口使用 `auth.getUserInfo()` 获取，用于鉴权和数据隔离。<br>2. **真实微信 openid (openid)**：在登录时通过 `uni.login()` 获取 `code`，后端调用 `jscode2session` 换取，用于微信支付等微信 API。 |
