...
CapsuleTabs 是横向**胶囊分段**标签栏，视觉参考 **shadcn/ui `@shadcn/tabs` 的 default 变体**（muted 槽 + 选中浮起块）。样式来自全局 `capsule-tabs.scss`（`.t-capsule-tabs*`）。

### 主要特性

- ✅ 全圆角胶囊轨道与选项，易扫读、易点击
- ✅ 支持 `value` 显式为 `null`（如「全部」筛选）
- ✅ 可选 `block`：均分整行宽度
- ✅ 需吸顶时用 **`StickyTabs` 包裹**（见下文）
- ✅ 跨平台：微信小程序、H5、App 等
