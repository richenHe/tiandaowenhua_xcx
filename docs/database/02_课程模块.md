...
CREATE TABLE courses (
  ...
  description VARCHAR(500) COMMENT '课程简介（纯文本摘要，检索用）',
  description_blocks MEDIUMTEXT NULL COMMENT '课程简介图文 JSON',
  content TEXT COMMENT '详细介绍（HTML）',
  outline TEXT COMMENT '课程大纲（纯文字 JSON 数组，兼容）',
...
| attend_count | INT | - | YES | 0 | 已上课次数（签到 +1） |
...
2. **价格非负**：original_price、current_price 必须 >= 0；复训费在 `class_records.retrain_price` 配置且 >= 0
3. **库存管理**：stock = -1 表示无限库存，>= 0 表示有限库存
4. **复训规则**：
   - attend_count 为已上课签到次数，初始为 0
   - 每次签到（扫码/后台单次/后台批量）attend_count + 1
   - attend_count >= 1 时再次预约需支付复训费
...
```sql
-- 插入课程数据
INSERT INTO courses (name, type, cover_image, description, original_price, current_price, allow_retrain, duration, status, sort_order) VALUES
('初探班', 1, 'https://example.com/course1.jpg', '天道文化入门课程，带您初探天道奥秘', 1688.00, 1688.00, 1, '2天', 1, 100),
('密训班', 2, 'https://example.com/course2.jpg', '深度密训课程，全面掌握天道文化精髓', 38888.00, 38888.00, 1, '5天', 1, 90),
...
(1, 'cloud-uid-001', 1, 1, '初探班', 'ORD202401150001', 1688.00, '2024-01-15 10:30:00', 0, 0, 1);

-- 插入用户课程数据（密训班购买，包含赠送的初探班）
INSERT INTO user_courses (user_id, user_uid, course_id, course_type, course_name, order_no, buy_price, buy_time, is_gift, attend_count, status) VALUES
(2, 'cloud-uid-002', 2, 2, '密训班', 'ORD202401150002', 38888.00, '2024-01-16 14:00:00', 0, 0, 1),
(2, 'cloud-uid-002', 1, 1, '初探班', 'ORD202401150002', 0.00, '2024-01-16 14:00:00', 1, 0, 1);
