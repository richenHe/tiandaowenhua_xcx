  ...
  **③ 修改方案**（说明打算怎么改，改了会影响什么）
  ...
  | 2026-04-08 | `CourseHomeCard.vue`；`tdesign-vars`；`pages/index/index` | **首页课程卡 UI 二次调整**：独立组件 + 底部仅类型名+价格；内容区 **`#FBF7EE`**；外圆角 **40rpx**；右侧胶囊**白底描边**、文案居中；**CTA 固定「查看详情」**（不做「N天后」）。 | MP-合规 MP.6 手测样式 |
  | 2026-04-08 | `course/handlers/public/getList.js`；小程序 `pages/index/index` | **课程列表补充 `next_class_date`**：对本页课程批量聚合 `class_records`（`status=1`、`class_date`≥今日北京日期）取最早 `class_date`；**首页 CTA 不读该字段**，仅接口预留/其它端可用。 | 部署 `course` 后 F2 列表字段 |
  ...
  **各流程必须检查的典型场景维度：**
  ...
  | MT-17 | **小程序申请退款（未签合同）**：用户在客服页点击"退款"→ 选择已支付未签合同的课程 → 填写原因 → 提交，`orders.refund_status` 应变为 1 | F13 S13.1 | user_id=30，存在 pay_status=1 且 contract_signed=0 的课程订单 | 小程序客服页→退款→选课程→填原因→提交 | `SELECT order_no, refund_status, refund_reason, refund_amount FROM tiandao_culture.orders WHERE user_id=30 AND refund_status=1 ORDER BY id DESC LIMIT 3` | ✅ 已通过（2026-03-04，user_id=32，order_id=61「更新课程_测试」¥1，refund_status=1，refund_reason="测试退"） |
  | MT-17a | **客服页 · 展示与文案（微信审核）**：页面为人工/工单引导，不出现「智能」「AI」「人工智能」「深度合成」等字样；标题为「客服」，入口图标为耳机类客服意象；快捷入口「意见反馈」「退款」「联系客服」可用；点击「联系客服」后在会话区追加常用/备用微信客服二维码（静态图），点击图片可 `previewImage`；点击底部占位条 toast 引导使用意见反馈 | F13 / 合规展示 | 任意已登录用户 | 我的→客服 | 肉眼检查：① 导航标题与页面内无「智能/AI/人工智能」；② 说明文案为人工联系、退款与加企微/微信客服指引；③ 无「内容由 AI 生成」类标识（本页非 AI 生成场景）；④「联系客服」后出现两张二维码且文案区分常用/备用 | ⏳ 重测（2026-04-08：新增联系客服二维码；2026-04-06：纯客服表述） |
  ...
  **⛔ 凡前置条件已满足的 F 类场景，必须在当前轮次内立即补测，不得推迟到下一次对话。**
  ...
  | `getCalendarSchedule` 日期范围 | `2026-03-01~03-31` | `SELECT MIN(class_date),MAX(class_date) FROM tiandao_culture.class_records WHERE status=1`；**2026-04-08**：接口按 `class_date`～`class_end_date`（含）展开每日键，返回 `nicknames[]`；跨月跨度须在对应月份请求内各验一日 |
